<template>
  <q-card class="list-card" bordered>
    <q-inner-loading :showing="loading" label="Criando cobrança..." label-class="text-teal"
      label-style="font-size: 1.1em" />
    <q-card-section>
      <div class="text-h6">Criar cobrança Pix</div>
      <div class="q-pa-md" style="max-width: 650px">
        <q-form ref="form" @submit="onSubmit" @reset="onReset" class="q-gutter-md" v-show="showForm">
          <q-input filled v-model="nomeDevedor" label="Nome devedor *" hint="Nome do devedor" lazy-rules :rules="[
            (val) => (val && val.length > 0) || 'Por favor, digite algo.',
          ]" />

          <q-input filled v-model="cpfDevedor" label="CPF devedor *" hint="CPF do devedor" lazy-rules :rules="[
            (val) =>
              (val !== null && val !== '') || 'Por favor, digite o CPF.',
          ]" />

          <q-input filled type="number" v-model="valor" label="Valor da cobrança *" hint="Valor" lazy-rules :rules="[
            (val) =>
              (val !== null && val > 0) || 'Por favor, digite o Valor.',
          ]" />

          <q-input filled v-model="solicitacaoPagador" label="Solicitação ao Pagador *" hint="Solicitação ao Pagador"
            lazy-rules :rules="[
              (val) =>
                (val !== null && val !== '') ||
